...
function EventDetails({ item, isOwner, saving, onEdit, onRespond, onGoDate }) {
  ...
  return <div className="cal-detail">
    ...
    <div className="cal-detail-time"><strong>{timeText(item.starts_at)}{!!item.duration_minutes && <span>—{endTime(item)}</span>}</strong><p>{dayText(item.starts_at, { weekday: 'long', year: 'numeric' })}<span> · {item.is_journal_only ? 'время журнала' : 'время расписания'}</span></p></div>
