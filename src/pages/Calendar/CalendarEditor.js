...
export default function CalendarEditor({ editor, context, today, saving, error, onSubmit, onCancel }) {
  ...
  return <form className="cal-editor" onSubmit={submit}>
    ...
    {item && <div className="cal-editor-origin"><strong>{item.branch_name}</strong><span>{dayText(item.starts_at)} · {timeText(item.starts_at)}–{endTime(item)}</span></div>}
