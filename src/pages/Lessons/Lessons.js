...
export default function Lessons() {
  ...
  return <Layout headerTitle={title} className="layout-lessons-workspace">
    <div className="lesson-workspace">
      ...
      <Modal isOpen={showInvoices} onClose={() => setShowInvoices(false)} title="Счета по филиалам" size="medium"><div className="lc-invoices"><p>По занятиям за {new Date(`${month}-01T12:00:00`).toLocaleDateString('ru-RU', { month: 'long', year: 'numeric' })}. Выберите филиал, затем подтяните полный расчёт за месяц в редакторе счёта.</p>{groups.map(group => <button key={group.id} onClick={() => { setShowInvoices(false); navigate(`/accounting/invoices?month=${month}&branch_id=${group.id}&create=1`); }}><span><strong>{group.name}</strong><small>{countLabel(group.lessons.length, ['занятие', 'занятия', 'занятий'])}</small></span><strong>{formatCurrency(totalsFor(group.lessons).revenue)}</strong><DashIcon name="arrow" /></button>)}</div></Modal>
    </div>
  </Layout>;
}
